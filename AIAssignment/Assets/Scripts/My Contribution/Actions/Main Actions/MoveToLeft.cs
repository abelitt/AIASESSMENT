...
public class MoveToLeft : AIAction {
    ...
    {
        if (Vector3.Distance(GameObject.Find("PowerupSpawner").transform.position, agent.transform.position) < 2)
        {
            return true;
        }
        else
...
//GameObject.Find("PowerupSpawner").transform.position.x == agent.transform.position.x && GameObject.Find("PowerupSpawner").transform.position.z == agent.transform.position.z